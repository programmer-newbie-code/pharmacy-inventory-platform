...
| --- | --- | --- | --- |
| Branch, signed-commit, PR, CI workflow | Shipped | `AGENT.md`, `.github/workflows/ci.yml`, and PR #143: analyzer, coverage, Windows, Android, signatures, and secret scan all passed before merge; main run `31539887141` passed for `8d4da0e`. | Keep enforcing the workflow for every increment. |
...
| Current report-export audit status | Shipped for best-selling, procurement, cash movement, and classic sales | PR #130 added best-selling Excel export/audit. PR #132 added filtered, deterministic procurement and cash-movement Excel exports, UI feedback, and save-before-audit records. PR #135 added save-before-audit classic sales Excel exports. | SIPNAP currently ignores its selected period and receipt quantities; prescription CSV emits blank data rows. Both require a separate regulated-report design decision and implementation increment. |
...
| POS, returns, shifts, receipts, printer recovery | Partial | POS, return, shift, receipt PDF/storage, and targeted tests are present. PR #139 adds per-action busy state plus retryable, localized save and print failure recovery. PR #143 attributes shift/cash-movement actions to the active session rather than fallback user `1`, blocks missing sessions, and localizes those workflows. | Verify price-below-cost authorization, close-shift supervisor review, and physical-printer recovery on a real Windows/Android build. |
...
| Google Drive backup and desktop OAuth | Partial / external verification required | Desktop OAuth configuration, credential store, upload client, and tests exist; Windows now displays configuration guidance instead of a plugin exception. PR #141 makes an enabled automatic Drive backup record a `drive` / `Failed` log when a restart leaves no active in-memory OAuth account, instead of silently skipping upload. | Verify real Android and Windows OAuth with an authorized Google Cloud configuration and account. Re-authenticate through the existing Backup action after a restart; no background OAuth or token persistence is claimed. |
...
| Reports, financial exports, alerts | Shipped for best-selling, procurement, cash movement, and classic sales; Partial overall | Sales summary, procurement, cash movement, SIPNAP, and best-selling export services exist; `SalesAnalyticsScreen` has Today/Week/Month/custom presets, ranking modes, and export. | Correct the proven SIPNAP and prescription-CSV defects only after the report is scoped against official SIPNAP fields and the application's available movement data. |
...
1. Obtain a regulated-report decision, then correct the proven SIPNAP-period/receipt and prescription-CSV blank-row defects in a dedicated spec/plan and PR.
2. Perform evidence-led Google Drive/backup and barcode audits. The reproducible auto-Drive silent-skip path is fixed in PR #141; request OAuth or physical-device access only where verification cannot be reproduced.
...
4. Complete visual QA of Windows, Android, and GitHub Pages using published/release builds; keep subjective redesigns for product review.
