...
class BackupDocument {
  ...
  /// Collections added after schema version 2. Older backups may omit them.
  static const optionalCollections = <String>['csvImportLogs'];
    ...
    for (final collection in requiredCollections) {
      ...
      if (rawRows is! List) {
        throw BackupValidationException(
            'Backup collection "$collection" is missing or invalid.');
      }
      data[collection] = rawRows.map((row) {
        if (row is! Map) {
          throw BackupValidationException(
              'Backup collection "$collection" contains an invalid row.');
        }
        return Map<String, Object?>.from(row);
      }).toList(growable: false);
    }

    for (final collection in optionalCollections) {
      final rawRows = rawData[collection];
      if (rawRows == null) {
        data[collection] = [];
        continue;
  ...
  static void _validateCounts(
    ...
    if (rawCounts is! Map) {
      throw BackupValidationException(
          'Backup record counts are missing or invalid.');
    }
    for (final collection in requiredCollections) {
      final count = rawCounts[collection];
      if (count is! int || count != data[collection]!.length) {
        throw BackupValidationException(
            'Backup count for "$collection" does not match its records.');
      }
    }
    for (final collection in optionalCollections) {
      final rows = data[collection]!;
      final count = rawCounts[collection];
      if (count != null && (count is! int || count != rows.length)) {
        throw BackupValidationException(
          'Backup count for "$collection" does not match its records.',
        );
      }
    ...
    for (final entry in data.entries) {
      ...
      for (final row in entry.value) {
        ...
        if (id is! int || id <= 0 || !ids.add(id)) {
          throw BackupValidationException(
              'Backup collection "${entry.key}" contains a duplicate or invalid ID.');
        }
      }
  ...
  static void _validateReferences(
    ...
    _reference(data['products']!, 'storageLocationId', ids['storageLocations']!,
        nullable: true);
    ...
    _reference(
        data['returnTransactions']!, 'originalTxnId', ids['saleTransactions']!);
    ...
    _reference(
        data['purchaseOrderItems']!, 'purchaseOrderId', ids['purchaseOrders']!);
  ...
  static void _reference(
    ...
    for (final row in rows) {
      ...
      if (value is! int || !targetIds.contains(value)) {
        throw BackupValidationException(
            'Backup reference "$field" is invalid.');
      }
    }
  }
}
